...
<!-- SIDEBAR -->
    ...
    <ul class="list-unstyled components">
						...
						@if ($categories && count($categories))
							...
							<li  data-category="{{$category->href}}">
									<a href="javascript: void(0);">
											{{ $category->title }}
									</a>
							</li>
    ...
    <ul class="list-unstyled buttonsGroup-public">
        ...
        @else
        <li>
            <a href="javascript:;"  data-toggle="modal" data-target="#login-modal" class="btn btn--logout modal-btn primaryBtn">Login</a>
        </li>
        <li>
            <a href="javascript:;"  data-toggle="modal" data-target="#register-modal" class="btn btn--logout modal-btn primaryBtn">Register</a>
        </li>
        @endif
    </ul>
</nav>
        ...
        <div class="col-10 col-sm-6 text-sm-right text-center">
            <div class="btn primary-btn d-inline-block mt-1 user-info">
							<span class="custom-input-group-icon text-left" style="margin-right: 10px;" data-toggle="tooltip"
								...
								<span id="user-username">{{ Auth::check() ? Auth::user()->username : '--' }}</span>
							</span>
                <span class="custom-input-group-icon text-left" style="margin: 10px;" id="hideMsg" data-toggle="tooltip"
                      title="Balance">
								<i class="fas fa-wallet"></i>
								<span id="user-balance">{{ Auth::check() ? number_format(Auth::user()->balance, 2,".",",") : '--' }} @if( Auth::check() && auth()->user()->present()->shop ){{ auth()->user()->present()->shop->currency }}@endif</span>
							</span>
                ...
                <span class="custom-input-group-icon text-left" style="margin-left: 10px;" data-toggle="tooltip"
                      title="Bonus">
								...
								<span id="user-cashback">{{ Auth::check() ? number_format(Auth::user()->count_return, 2,".",",") : '--' }} @if( Auth::check() &&auth()->user()->present()->shop ){{ auth()->user()->present()->shop->currency }}@endif</span>
							</span>
            ...
            <div>
                <ul class="navbar-nav">
									...
									@if ($categories && count($categories))
										...
										<li class="nav-item" data-sv="{{$category->href}}" data-category="{{$category->href}}">
												<a class="nav-link text-center" href="javascript: void(0);">
														{{ $category->title }}
												</a>
										</li>
							...
							<nav class="navbar navbar-expand p-0 scrollable-navbar" id="vendors">
								<ul class="navbar-nav">
									@if ($categories && count($categories))
										@foreach($categories AS $index=>$category)
										@if (in_array($category->href, $companies))
										<li class="nav-item mr-2 ml-2 box active" data-category="{{$category->href}}">
...
<script>
	...
	var GLOBAL_GAMES_LIST = [
		...
		{
			"game_id":{{$game->id}},
			"launchUrl":"{{ Auth::check() ? route('frontend.game.go', $game->name).'?api_exit=/' : '' }}",
			"providerId":"bomba",
...
<div class="modal fade" id="login-modal" tabindex="-1" role="dialog" aria-labelledby="login-modal-title"
     aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document" id="login-modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body p-3">
                <form id="login-form" action="<?= route('frontend.auth.login.post') ?>" method="post">
                    ...
                    <button type="submit" form="login-form" value="@lang('app.log_in')" class="btn primary-btn w-100 mt-4">Log In</button>
                </form>
            </div>
        </div>
    </div>
...
<script src="/frontend/Tropicoblack/js/custom.js"></script>
<script type="text/javascript">
$(document).ready(function(){
	$('[data-category="all"]').click();
    $('li[data-category]').click(function () {
        window.location.href = "/categories/" + $(this).data('category');
    });
})
</script>
</body>
